...
| Part 4 — 출력/공유 | PDF·단톡방 공유 | ✅ 인스타 스토리 실제 게시 → `스토리/게시_인스타스토리_스크린샷.jpeg` (보너스 완성엔 동기 명함 리액션 별도) |

## 3. 디자인 판단 — 풀컬러 블루 카드 (사용자 선택)

- **앞면 = 토스 블루 풀블리드**: 배경을 `#3B8BFF→#1B64DA` 블루로 꽉 채우고 흰 글씨로 강한 컬러 임팩트.
  `about-me.md` §3 "디자인은 토스(Toss) 스타일 참고"에서 도출.
- **앞↔뒤 톤 반전**: 앞(풀블루) ↔ 뒤(화이트). 뒤집었을 때 "다른 면"이 즉시 인지된다.
- **폰트 2개**: Pretendard(이름·직함) + JetBrains Mono(email·github). 연락처를 모노로 쓴 이유는
  이메일·URL은 **한 글자만 틀려도 안 되는 정보**라 자간 고른 서체가 오독을 막기 때문.
- **강조 1개**: 앞면은 "실용주의 빌더" 한 구절에만 흰 하이라이트.
- **비율(배경 대비) 균형** — 재작업 포인트: 초안은 앞뒤 모두 가운데가 비어 배경 대비 비율이 떴다.
  - 앞면: 우측에 대형 **"S" 모노그램 패널 + `AI BUILDER`** 세로 라벨을 넣어 우측 여백을 채우고,
    콘텐츠를 상(칩·지역)/중(이름·직함)/하(연락처) 3단으로 고르게 분배.
  - 뒷면: 빈 중앙에 **일하는 방식 칩 4개**(작게 만들고 확인·꼼꼼함·정리정돈·깔끔한 UI, `about-me.md §7` 근거)를
    넣어 상(슬로건)/중(칩)/하(실적·QR)로 꽉 채움.
