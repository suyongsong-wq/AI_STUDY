...
| Part 4 — 공유 | 단톡방/인스타 공유 | ✅ 인스타 스토리 실제 게시 → `스토리/게시_인스타스토리_스크린샷.jpeg` (보너스 완성엔 동기 작품 리액션 별도) |
...
시그니처 원두백은 **브랜드 톤에 맞춰 직접 벡터(SVG)로 제작**했다(`생성이미지/ember_원두백.svg`).
매트 블랙 봉투 + 앰버 라벨(SIGNATURE BLEND · 200G) + `ROASTED TO ORDER` 스탬프 + 원두 알갱이.
Ember의 미니멀·헤어라인 톤에는 벡터가 정확히 맞고, **색·폰트가 브랜드와 100% 일치하며 재현 가능**하다.
